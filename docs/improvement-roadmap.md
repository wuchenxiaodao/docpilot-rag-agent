...
- [x] 12. 端到端生成质量评测（忠实度 / 引用正确性 / 拒答准确率，现有评测只覆盖检索层）（2026-08-25 完成：`scripts/eval_generation.py` 走生产同链路（rag-assistant 图全流程）三维度统一报告。拒答区分 truly_out 8 题（08-23 拒答分析确认）应拒答与 stale 标签 7 题（#4 在线入库后实际可答，只观察不计分，避免标签腐烂稀释指标）；引用两级机械判定——chunk 级（state.citations 与标注 expected_chunk_ids 交集=引对证据）+ 来源级（回答文内 Sources: 段解析，被引名须出现在检索返回=无捏造文件）；忠实度 LLM-as-judge 复用本地 Qwen 自评（supported/partial/unsupported，判官带问题与去 CITATIONS_JSON 的证据，鲁棒 JSON 抽取；同模型自评有偏，报告已注明仅作相对基线），--skip-judge 可跳过。单题 900s 超时（历史单题 7 小时离群）+ 逐题明细 JSONL 落盘防跑挂丢档。无新依赖，纯离线脚本不动产品代码。辅助函数 25 项单测（不需 Ollama），全套 269 通过零退化；本轮 Ollama 未启动，全量线上报告待服务在线后补跑。详见 README「Generation quality eval」节）
